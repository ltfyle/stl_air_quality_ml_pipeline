...
def get_weather_df():
	...
	df_weather = pd.concat(location_data, ignore_index=True)
	df_weather = df_weather.sort_values(["latitude","longitude","date"]).reset_index(drop=True)
	

	# simple agregation for each variable, by date
	df_weather_transformed=df_weather.groupby(['date'])[['temperature_2m_max','precipitation_sum','precipitation_hours','uv_index_max','wind_speed_10m_max','shortwave_radiation_sum']].mean().reset_index()
	df_weather_transformed=df_weather_transformed.set_index('date')
	
	return df_weather_transformed
